...
func Forge() (string, Signature, error) {
	...
	for k := 0; k < len(sigslice); k++ {
		...
		//Get the kth signature from sigslice array
		sig = sigslice[k]
		//Hash the signature so that we can compare it with the public key
		for i := 0; i < 256; i++ {
	...
	for c != 256 {
		...
		counter += 1
		msgString = "forged by Akshay" + strconv.Itoa(counter)
