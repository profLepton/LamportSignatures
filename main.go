...
func (Pkey PublicKey) ToHex() string {
	// format is zerohash 0...255, onehash 0...255
	var s string
	for _, zero := range Pkey.ZeroHash {
		s += zero.ToHex()
	}
	for _, one := range Pkey.OneHash {
		s += one.ToHex()
	}
	return s
}
...
func Verify(msg Message, pub PublicKey, sig Signature) bool {
	...
	// Your code here
	// ===
	//Dummy signature to store the public keys as needed for verifying the message.
	var baseline_pub Signature
	indices := [8]int{128, 64, 32, 16, 8, 4, 2, 1}

	for i := 0; i < 32; i++ {
		for j := 0; j < 8; j++ {
			index := 8*i + j
			if int(msg[i])&(indices[j]) != 0 {
				baseline_pub.Preimage[index] = pub.OneHash[index]
			} else {
				baseline_pub.Preimage[index] = pub.ZeroHash[index]
			}
		}
	}

	for i := 0; i < 256; i++ {
		if !sig.Preimage[i].IsPreimage(baseline_pub.Preimage[i]) {
			return false
		}
	}
	// ===
